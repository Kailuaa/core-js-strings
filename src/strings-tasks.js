...
function isPalindrome(str) {
  const rep = str.replace(/[^a-z]/gi, '').toLowerCase();
  return rep === rep.split('').reverse().join('').toLowerCase();
}
...
function encodeToRot13(str) {
  const upperСase = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (upperСase.includes(char)) {
      const index = upperСase.indexOf(char);
      const rotatedIndex = index < 13 ? index + 13 : index - 13;
      result += upperСase[rotatedIndex];
